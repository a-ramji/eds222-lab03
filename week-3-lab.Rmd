...
ggplot(data = pie_crab, aes(y = size, x = latitude)) +
  geom_point() +
  labs(x = "Latitude",
       y = "Size of crab carapace (mm)")+ theme_bw()

ggplot(data = pie_crab, aes(y = water_temp, x = latitude)) +
  geom_point() +
  labs(x = "Latitude",
       y = "Water temperature (Celsius)")+ theme_bw()
...
```{r}
# regression, linear model, size to latitude
mod_size <- lm(size ~ latitude, data = pie_crab)

mod_water <- lm(water_temp ~ latitude, data = pie_crab)
summary_water <- summary(mod_water)
summary_water$coefficients
R2_size <- summary(mod_size)$r.squared
R2_water <- summary(mod_water)$r.squared

print(paste0("R2 of size  latitude is:", round(R2_size, 2)))

print(paste0("R2 of water temp latitude is:", round(R2_water, 2)))

```

In our data, 35% of the variation in carapace size can be explained by latitude
In our data, 92% of the variation in water temperature can be explained by latitude
...
- how vintage effects fuel efficiency
...
class(mpg$year)
# we want to change this from integer/numeric to character/class
# we don't have a per year interpretation that we want

mpg_refactored <- mpg %>% mutate(year = as.factor(year))
class(mpg_refactored$year)
...
# 2 boxes for each of our 2 vintages

ggplot(data = mpg_refactored, aes(x = year, y = hwy)) +
  geom_boxplot() +
  labs(x = "year",
       y = "fuel efficiency") +
  theme_bw()
...
**Answers:**
```{r vintage_regression}
model_year <- lm(hwy ~ year, mpg_refactored)
model_year %>% summary()
```
...
- many other factors going on, might want to include more variables in our model
- 
y = beta-naught + beta-1 * x + u (error term)

OVB:Omitted variable(s) Bias:

occurs if...
1. another variable z is correlated with x
AND
2. z influences y
...
$$\text{y} = \beta_0 + \beta_1 \text{x}_1 + \beta_2 \text{x}_2 + \epsilon$$
...
- try controlling for engine size, seeing how this impacts our regression

**Answers:**
```{r}
ggplot(data = mpg_refactored, aes(x = displ, y = hwy)) +
  geom_point(color = "seagreens") +
...
multi_model_mpg <- lm(hwy ~ year + displ, data = mpg_refactored)

multi_model_mpg %>% summary()
```
...
**Answer:**
- when the year is 1999 AND the displacement is zero 0, your hwy is estimated to be 35.27mpg
- when the year is 2008 and the displacement is zero, your hwy is estimated to be 1.4mpg higher than 35.27mpg (36.67ish)
- predicted mpg for 1999 vintage with no engine displacement (=0) is 35.276
- predicted mpg for 2008 vintage with no engine is 36.67mpg
- 1.4 is if engine size is fixed, 2008 vintage fuel efficiency is 1.4 units higher than 1999 vintage
- evidence that there is omitted variable bias
- for each liter increase in engine size, mpg on average decreases by 3.611mpg, holding vintage constant
- "holding vintage constant" in the data, we have a lot of characteristics, mpg, finding 2 cars with the same vintage, only looking at difference b/w engine displacement
- **parallel slopes model**
- estimating one relationship on average b/w engine displacement and vintage
- average treatment effect
- interaction affect -- could allow slope to vary by vintage (next week), multiply the 2 variables etc. 
- looking at pairs of vehicles -- holding vintage fixed (w/in same year)
-  engine displacement and vintage together explain 60% of the variation in mpg (?)

"all else equal"

- can we learn about relationship between vintage and engine size?
- wehn we didn't control for engine size, rly small vintage effect
- control for displ, big vintage effect
- engine size is getting ___ w vintage effect
- bigger, negative effect, drives down the estimate 
- effect of engine getting bigger is pulling down mpg
- vintage ffect 2008 pushing it up
- when we didnt control, smaller effect
- when displ wasn't in the model, there was a positive correlation where larger dipsl seen in later vintage years
- higher / larger engine sizes were happening at the same time in the 2008 vintages
- could check b/w vintage size and displacement
- as the vintage increases, we would expect to see engine size increase
- vintage effect got bigger when we controlled for engine size
...
mpg_refactored %>% 
  ...
  labs(x = "Engine Displacement, in litres",
       y = "Highway Miles per gallon",
       title = "Engine Displacement & Mileage for 1999 and 2008 vintages") +
  theme_bw()

```
- on avg, looks like blue dots are higher than red dots, showing 2008 vintages have better mpg
...
```{r, echo=TRUE, cache=T}
mod <- lm(hwy ~ year + displ, data = mpg_refactored)
augment(mod)
...
mpg_refactored %>% 
  ...
  theme_bw()
```
- estimates 1 relationship w displacement, different intercepts for the 2 years
- slope is the -3.6 from the multiple regression model earlier (parallel slopes model)
